...
(async function () {
	...
	commander
		...
		.option('-s, --senderstartingbalance <Staring Balance of Sender Account>', 'Overwriting value.', '0')
		.parse(process.argv);
	...
	const senderStartingBalanceHash = poseidon.F.toString(poseidon([senderStartingBalance]));
	console.log(`senderStartingBalance='${senderStartingBalance}'; senderStartingBalanceHash='${senderStartingBalanceHash}'`);
	const senderEndingBalanceHash = poseidon.F.toString(poseidon([senderEndingBalance]));
	console.log(`senderEndingBalance='${senderEndingBalance}'; senderEndingBalanceHash='${senderEndingBalanceHash}'`);
